...
const NGOProfile = ({ mode = "profile" }) => {
  const { user } = useAuth();
  const { ngoId: routeNgoId } = useParams();
  ...
  // Helper to get NGO ID string
  const getNgoId = () => {
    if (isVisitor) return routeNgoId;
    if (user?.ngo?._id) return user.ngo._id;
    if (typeof user?.ngo === "string") return user.ngo;
    return null;
  };

  // Check if user follows the NGO (visitor only)
  ...
  // Toggle follow/unfollow NGO (visitor only)
  ...
  // Fetch NGO profile and gamification data
  const fetchNGO = async () => {
    const idToFetch = getNgoId();
    if (!idToFetch) {
      console.warn("No valid NGO ID found to fetch profile");
      return;
    }
    try {
      ...
      const pointsRes = await axios.get(`${BaseURL}/api/v1/ngo/${idToFetch}/points`, {
        headers: { Authorization: `Bearer ${token}` },
      });
          ...
          <div className="bg-white shadow-md rounded-xl p-5 space-y-3 lg:sticky lg:top-24 self-start flex flex-col justify-start">
            ...
            <div className="mt-3">
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                {ngo?.category?.length > 0 ? ngo.category.map((cat, idx) => (
            ...
            <div>
              <iframe
                ...
                src={`https://www.google.com/maps?q=${ngo?.latitude || 28.6139},${ngo?.longitude || 77.209}&hl=es;z=14&output=embed`}
                loading="lazy"
              />
            </div>
          </div>
                  ...
                  <Tab
                    ...
                    className={({ selected }) =>
                      `px-4 py-2 text-sm font-semibold border-b-2 ${
                        selected ? "border-[#19398a] text-[#19398a]" : 
                    ...
                    <div className="mt-6">
                      ...
                      {nextBadge && (
                        <div
                          ...
                          aria-valuemin={0}
                          aria-valuemax={nextBadge.points}
                        >
                          ...
                          <div className="w-full bg-gray-300 h-3 rounded-full overflow-hidden">
                            <div
                              className="h-3 bg-gradient-to-r from-amber-400 via-yellow-300 to-lime-200 transition-all duration-700"
