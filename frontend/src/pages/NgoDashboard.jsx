...
const NgoDashboard = () => {
  ...
  const [scheduleModal, setScheduleModal] = useState(null); // handle ID of request being scheduled
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BaseURL}/api/requests/ngo`, {
  ...
  return (
    <div style={{ fontFamily: "'Inter', Arial, sans-serif", background: "#f4f6f8", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      <div style={{ flex: 1, padding: "0rem", maxWidth: 1000, margin: "0 auto" }}>
        ...
        {requests.length === 0 ? (
          <p style={{ textAlign: "center" }}>No incoming requests yet.</p>
        ) : (
            ...
            {requests.map((req) => (
              <div key={req._id} style={{ background: "#fff", borderRadius: 20, padding: "2rem", boxShadow: "0 10px 25px rgba(0,0,0,0.08)" }}>
                <h3 style={{ fontSize: "1.5rem", fontWeight: 600, color: "#0f2a66", marginBottom: "0.5rem" }}>
                  {req.request_id.full_name}
                </h3>
                <p><strong>Category:</strong> {req.request_id.category}</p>
                <p><strong>Priority:</strong> {req.request_id.priority}</p>
                <p><strong>Description:</strong> {req.request_id.description}</p>
                <p><strong>Address:</strong> {req.request_id.address}</p>
                <p><strong>Phone:</strong> {req.request_id.phone || "N/A"}</p>
