...
const Don_Notification = () => {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BaseURL}/api/v1/donation_notification`, {
  ...
  const markAsRead = async (id) => {
    try {
      const token = localStorage.getItem("token");
      await fetch(`${BaseURL}/api/v1/donation_notification/${id}/read`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleClick = (notification) => {
    ...
    if (notification.referenceModel === "DonationHandling") {
      navigate("/user-donation");
    } else if (notification.referenceModel === "Donation") {
      navigate("/donations-management");
    } else {
...
export default Don_Notification;
